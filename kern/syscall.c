	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uint32_t)srcva >= UTOP || (uint32_t)srcva % PGSIZE 
		...
		return -E_INVAL;
	pte_t *p;
	struct PageInfo *pp = page_lookup(src->env_pgdir, srcva, &p);
	if(!pp)
		return -E_NO_MEM;
	...
	if(perm & PTE_W && !(*p & PTE_W))
		return -E_INVAL;
	if((ret = page_insert(dst->env_pgdir, pp, dstva, perm)) < 0)
		return ret;
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(e->env_ipc_dstva < (void *)UTOP && srcva < (void *)UTOP){
		if((uint32_t)srcva % PGSIZE)
			return -E_INVAL;
		pte_t *p;
		struct PageInfo *pp = page_lookup(curenv->env_pgdir, srcva, &p);
		if(!pp)
			return -E_NO_MEM;
